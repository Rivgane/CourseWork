...
int Laba1()
{
	...
	std::cout << "Personal task: 11" << std::endl << std::endl;

	int intc, N, F;
	std::cout << "Enter intc value: ";
	std::cin >> intc;
	F = intc;
	std::cout << "Before bit shifting: ";
	for (int i = 1; i <= order; i++)
	{
		putchar(intc & mask ? '1' : '0');
		intc <<= 1;
		if (i % order - 1 == 0)
		{
			putchar(' ');
	...
	intc = F;
	std::cout << "Enter bit shifting N value: ";
	std::cin >> N;
	intc <<= N;
	std::cout << "After bit shifting: ";
	for (int i = 1; i <= order; i++)
	{  
		putchar(intc & mask ? '1' : '0');
		intc <<= 1;
